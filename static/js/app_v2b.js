...
function init() {
  d3.json("https://static.bc-edx.com/data/dl-1-2/m14/lms/starter/samples.json").then((data) => {
    ...
    let trace2 = {
      ...
      marker: {
        size: sliced_values,
        //set 'sizeref' to an 'ideal' size given by the formula sizeref = 2.0*max(array_of_size_values) / (desired_maximum_marker_size ** 2)
        sizeref: 2.0 * Math.max(sliced_values) / (desired_maximum_marker_size**2),
        sizemode: 'area'
      }
    };
